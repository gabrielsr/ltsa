...
/** DrawMachine handles the drawing of machines in the Draw tab. */
public class DrawMachine implements IDrawMachine, Exportable {
	...
	public DrawMachine(ICompactState m, JPanel p, Font fn, Font fl, boolean dn,
	...
	public ICompactState getMachine() {
		return mach;
	}
	...
	protected Dimension computeDimension(ICompactState m) {
		...
		int pwidth = errorState == 0 ? 10 + nameWidth + STATESIZE + endWidth
				+ (m.getMaxStates() - 1) * SEPARATION : 10 + STATESIZE
				+ endWidth + m.getMaxStates() * SEPARATION;
	...
	public void draw(Graphics g) {
		...
		if (m.getMaxStates() > MAXDRAWSTATES) {
			...
			g.setFont(nameFont);
			g.drawString(m.getName() + " -- too many states: "
					+ m.getMaxStates(), topX, topY + 20);
		} else {
			// display name
			g.setFont(nameFont);
			FontMetrics fm = g.getFontMetrics();
			...
			for (int i = 0; i < m.getMaxStates(); i++) {
				...
				while (p != null) {
					...
					if (event.charAt(0) != '@')
						while (tr != null) {
							ntrans[tr.getNext() + 1]++;
							drawTransition(g, i, tr.getNext(), event, ntrans[tr
									.getNext() + 1], i == lastselected
									&& tr.getNext() == selected, false);
							tr = tr.getNondet();
						}
					p = p.getList();
				}
				...
				while (p != null) {
					...
					if (event.charAt(0) != '@')
						while (tr != null) {
							ntrans[tr.getNext() + 1]++;
							if (!newLabelFormat) {
								drawTransition(g, i, tr.getNext(), makeLabel(i, (EventState) tr), ntrans[tr.getNext() + 1], 
	...
	private String makeLabel(int fromState, EventState tr) {
		// TODO: No need to normalise probability, should always be normalised
		StringBuffer str = new StringBuffer();
		boolean isProbState = (((CompactState) mach).isProbState(fromState));
		
		// handle probabilistic transitions
		if (isProbState) {
			// find transition
			// java.util.Iterator i =
			// mach.probTransitions[fromState].iterator();
			IEventState e = mach.getStates()[fromState];
			// CompactState.ProbClockTransition pt = null;
			/*
			 * jew01 we only need the probability, no need to keep the whole
			 * transition. besides, there may now be multiple probabilistic
			 * transitions from state A to state B. We need to check this to get
			 * the right probability.
			 */
			double prob = 0;
			double sum = 0;
			while (e != null) {
				// CompactState.ProbClockTransition t =
				// (CompactState.ProbClockTransition) i.next();
				if (e.getNext() == tr.getNext() && e.getProb() == tr.getProb())
					prob = tr.getProb();
				sum += e.getProb();
				e = e.getList();
			}
			...
			if (prob != 0 && sum != 0) {
				str.append('(');
				// str.append( FORMAT.format( prob/sum ) );
				str.append(FORMAT.format(prob));
				str.append(") ");
			} else
				throw new RuntimeException("Probabilities can not be 0 !!"); // shouldn't
																				// ever
																				// happen
		}
		...
		if (tr.getCondition() != null) {
			str.append('?');
			str
					.append(tr.getCondition().prettyPrint(
							(StochasticAutomata) mach));
			str.append("? ");
		}

		str.append(mach.getAlphabet()[tr.getEvent()]);

		if (tr.getAction() != null) {
			String s = tr.getAction().prettyPrint((StochasticAutomata) mach);
			if (!s.equals("")) {
				str.append(" <");
				str.append(s);
				str.append('>');
	...
	private void drawTransition(Graphics g, int from, int to, String s, int n,
			boolean highlight, boolean dotext) {
		if (highlight)
			g.setColor(Color.red);
		else
			g.setColor(Color.black);
		...
		int w = (to != from) ? (SEPARATION * Math.abs(from - to))
				: SEPARATION / 3;
		int h = (to != from) ? (ARCINC * Math.abs(from - to)) : STATESIZE - 5;
		int y = zeroY - (h - STATESIZE) / 2;
		if (n == 1 && !dotext) { // only draw arc for first transition
									// to->from
			if (from != to) {
		...
		g.setColor(Color.white);
		g.fillRect(px, py - fm.getMaxAscent(), fm.stringWidth(s), fm
				.getHeight());
		if (highlight
	...
	private void initCompactLabels() {
		...
		for (int i = 0; i < mach.getMaxStates(); i++) {
			...
			while (current != null) {
				String[] events = EventState.eventsToNextNoAccept(current, mach
						.getAlphabet());
