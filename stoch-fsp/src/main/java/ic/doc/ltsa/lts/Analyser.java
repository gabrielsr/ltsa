...
public class Analyser implements IAnimator,IAutomata {
    ...
    protected CompositeState cs;                       // the composite being operated on
    private CompactState[] sm;                  	    // array of state machines to be composed
    private LTSOutput output;                     	    // interface for text output
    protected Hashtable alphabet=new Hashtable();     // map action name to bitmap of shared machines
    protected Hashtable actionMap = new Hashtable();  // map action name to new number;
    protected int [] actionCount;                     // number of machines which share this action;
    protected String[] actionName;                    // map number to name;
    ...
    // animation variables
    private ic.doc.ltsa.common.infra.EventManager eman;
    ...
    protected BitSet highAction = null;   //actions with high priority
    ...
    public static boolean partialOrderReduction = true;
    public static boolean preserveObsEquiv = true;

    /** true iff the analyser returned by {@link
     * #getInstance(CompositeState,LTSOutput,EventManager)} should
     * analyse extended systems. */
 

    // rbc&su2 - chagned this to false from true to see if determisation bug goes away
    public static boolean extendedAnalyser = false;
    
    public Analyser() {}
      
	
    /** Returns the composition that is being analysed.
     */
    protected CompositeState getCompositeState() {
	return cs;
    }
	
    /** Returns the component machines in the composition. */
    protected CompactState[] getMachines() {
	return sm;
    }

    private PartialOrder partial = null;

    public Analyser (ICompositeState cs,LTSOutput output,ic.doc.ltsa.common.infra.EventManager eman) {
	this(cs,output,eman,false);
    }

    public Analyser (ICompositeState pCs,LTSOutput output, ic.doc.ltsa.common.infra.EventManager eman, boolean ia) {
	if (pCs==null) ; // TODO: return ? error message ?
        this.cs = (CompositeState)pCs;
        this.output = output;
        this.eman   = eman;
	...
	canTerminate = terminating.containsAll(nonterminating);
        ...
        for (int i=0; i<sm.length; i++ )
            for(int j=0; j<sm[i].getMaxStates();j++) {
                IEventState p = sm[i].getStates()[j];
                while(p!=null) {
                    IEventState tr = p;
                    tr.setMachine( i );
    ...
    protected byte[] endseq() {
	int [] state = new int[Nmach+1];
	for (int i=0; i < Nmach ; i++) {  //rbc changed this to <= from just <
	    if(sm[i].endseq<0) return null;
	    state[i]=sm[i].endseq;
	}
	return coder.encode(state);
    }

    private CompactState private_compose(boolean pDoHiding) {

		output.outln("Composing at CompactState.private_compose...");
		long start = System.currentTimeMillis();
		int ret = newState_compose();
		
		System.out.println("Analyser.private_compose -> This is the composer used");
		CompactState c = new CompactState(cs.name, analysed, compTrans, actionName, endseq());
		if (pDoHiding) {
			if (cs.hidden != null) {
				if (!cs.exposeNotHide) {
					c.conceal(cs.hidden);
    ...
    List eligibleTransitions(int [] state) {
		...
		if (partial!=null) {
		    if (asteriskEvent>0 && EventState.hasEvent(sm[Nmach-1].getStates()[state[Nmach-1]],asteriskEvent))  {
			//do nothing
		    } else  {
        ...
        for (int i = 0; i<Nmach; i++)  {// foreach machine
            IEventState p = sm[i].getStates()[state[i]];
    ...
    protected void outStatistics(int states, int transitions) {
		...
		while (!analysed.empty()) {
			...
			if (analysed.marked()) {
				...
				if (stateCount % 10000 == 0) {
					...
					//print out statistics about progress every 10000 states
				...
				if (transitions == null) {
					...
					if (!isEND(state)) {
						...
						//if there are no transitions from the current state, but
						//it is not an end state, we have a deadlock
						if (!deadlockDetected) {
	...
	return EventState.hasEvent(sm[Nmach-1].getStates()[ds[Nmach-1]],acceptEvent);
    }
    ...
    //returns true if state is the end state
    ...
    // TODO: copy these to StochasticAnalyser and adjust...
    // Animator routines ---------------------------------------------------------
    private String [] menuAlpha;
    private Map actionToIndex ;  // maps action number(key) to index
    private Map indexToAction ;  //maps index(key) to action number
	...
	private void getMenu(List<String> a) {
		if (a!=null) {
			...
			for (Iterator<String> i = validAction.iterator(); i.hasNext();) {
				String s = i.next();
    ...
    public BitSet initialise(List<String> menu) {
        choices = eligibleTransitions(currentA=coder.decode(coder.zero()));  // set state to 0
        if(eman!=null) eman.post(new ic.doc.ltsa.common.infra.LTSEvent(LTSEvent.NEWSTATE,currentA)); //initialise animation to 0
        getMenu(menu);
    ...
    private int[] step(int action) {  // take step from current and post it
        ...
        if (eman!=null) {
            eman.post(new ic.doc.ltsa.common.infra.LTSEvent(LTSEvent.NEWSTATE,tmp,actionName[action]));
        }
        return tmp;
    }
    ...
    public void setPreserveObsEquiv(boolean pPreObsEq) {

        preserveObsEquiv = pPreObsEq;
    }
}
