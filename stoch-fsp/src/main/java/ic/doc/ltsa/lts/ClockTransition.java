...
class ClockTransition
     ...
     */
    public ClockTransition( int from, Symbol event, int to ) {
	this( from, event, to, null, null, null, null );
    }
     ...
     */
    public ClockTransition( int from, Symbol event, int to,
			    Collection conditions, Collection actions,
	...
	tr = new EventState( s.getEventNumber( event ), to );
	tr.condition = makeCondition( s );
	tr.action = makeAction( s );
	...
	while( i.hasNext() ) {
		...
		.add( cond,
		      new ClockCondition(s.getClockNumber(spec.getClockName()),
					 !spec.isNegated() ) );
	}
     ...
     */
    public void instantiateClocks( Collection conds,
				   Collection acts,
				   Hashtable locals,
				   Hashtable globals ) {
	if( conds != null ) conditions = conds;
	else conditions = new Vector();
	
	if( acts != null ) actions = acts;
