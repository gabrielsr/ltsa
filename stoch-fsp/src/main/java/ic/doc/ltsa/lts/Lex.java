...
/** This is the lexical analyser for (extended) FSP.
 *  It scans the input for tokens (Symbols) and fetches
 *  the next one on command.
 * */
...
private void in_number () {
	...
	int digit = 0;
	int base = 10;
	boolean badDoubleDef = false;

	symbol.kind = Symbol.INT_VALUE;			// assume number is a INT

	// check for definitions of form .12
	if( ch == '.' ) {
		badDoubleDef = true;
	}
	...
	if (base == 10) {  // determine if real
		/* Code block reinserted from comment */
...
private void in_string () {
	...
	do {
		next_ch ();
	/*	if (ch == '\\')  
			in_escseq (); */  //no esc sequence in strings
