...
public class SuperTrace implements ISuperTrace {
    ...
    private LinkedList errorTrace;

    private final static int SUCCESS = 0;
    private final static int DEADLOCK = 1;
    private final static int ERROR = 2;

    private int DEPTHBOUND = 100000;
    private int HASHSIZE = 8000; //Kilobytes
    ...
    private boolean contains(byte[] key) {
        ...
        return table.get(hashOne(key) % nbits) && table.get(hashTwo(key) % nbits);
    }
    ...
    private int search() {
        ...
        while (!stack.empty()) {
            if (stack.marked()) {
                ...
                if (transitions.isEmpty() && !mach.END(currentState))
                    return DEADLOCK;
    ...
    public void setOutput(LTSOutput pWindow) {

       output = pWindow;
    }

    public void setAnalyser(IAutomata pMach) {
