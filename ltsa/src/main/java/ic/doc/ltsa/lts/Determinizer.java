...
public class Determinizer {
    ...
    protected CompactState makeNewMachine() {
        ...
        m.setAlphabet(new String[machine.getAlphabet().length]);
        for (int i=0; i<machine.getAlphabet().length; i++) m.getAlphabet()[i]=machine.getAlphabet()[i];
        m.setMaxStates(nextState);
		...
		if (machine.getEndseq()>=0) 
		{
			...
			es.set(machine.getEndseq());
			Integer ii = (Integer)map.get(es);
            if (ii!=null) m.setEndseq(ii.intValue());
		}
        output.outln("DFA("+machine.getName()+") has "+m.getMaxStates()+" states.");
        return m;
    }
