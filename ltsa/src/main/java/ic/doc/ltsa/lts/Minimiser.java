    ...
    public CompactState minimise() {
         ...
         if (machine.getEndseq()>=0) {
            ...
            machine.getStates()[es] 
             = EventState.add(machine.getStates()[es],new EventState(machine.getAlphabet().length,es));
         }
         if (machine.hasTau()) {
             initTau();
    ...
    private CompactState makeNewMachine() {
        ...
        if (machine.getEndseq()<0) 
          ...
          m.getStates()[m.getEndseq()] 
             = EventState.remove(m.getStates()[m.getEndseq()],new EventState(m.getAlphabet().length,m.getEndseq()));
        }
