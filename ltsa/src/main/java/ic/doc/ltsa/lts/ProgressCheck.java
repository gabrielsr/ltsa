...
public class ProgressCheck implements IProgressCheck {
    ...
    private Stack stack;
    private int id = 0;
    private int ncomp = 0; // number of connected components
	...
	private IFluentTrace tracer = null;
	...
	public ProgressCheck(IAutomata m, LTSOutput o, IFluentTrace t) {
        mach = m;
        output = o;
		tracer = t;
    }
    ...
    private void findCC() { // non-recursive version
        ...
        mach.setStackChecker(hh);
    	  ...
    	  while (!stk.empty()) {
                ...
                else if (accept<0 || transitions.getAction()!=accept) //ignore accept label transitions
                {
